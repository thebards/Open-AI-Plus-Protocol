...
makeSuiteCleanRoom('Upgradeability', function () {
	...
	// We're going to validate the first 31 slots and the 32rd slot before and after the change
	it("Should upgrade and set a new variable's value, previous storage is unchanged, new value is accurate", async function () {
		...
		let prevNextSlot = await ethers.provider.getStorageAt(proxyHub.address, 32);
		...
		const newNextSlot = await ethers.provider.getStorageAt(proxyHub.address, 32);
