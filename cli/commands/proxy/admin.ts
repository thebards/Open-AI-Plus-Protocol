...
export const setProxyAdmin = async (cli: CLIEnvironment, cliArgs: CLIArgs): Promise<void> => {
	...
	// Get the proxy admin
	const proxyAdminEntry = cli.addressBook.getEntry('BardsProxyAdmin')
	if (!proxyAdminEntry || !proxyAdminEntry.address) {
		logger.crit('Missing BardsProxyAdmin configuration')
		return
	}
	const proxyAdmin = getContractAt('BardsProxyAdmin', proxyAdminEntry.address).connect(cli.wallet)

	// Change proxy admin
	await sendTransaction(cli.wallet, proxyAdmin, 'changeAdmin', [
