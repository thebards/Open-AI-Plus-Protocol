...
import { parseGRT, formatGRT } from '../utils'
...
export const airdrop = async (cli: CLIEnvironment, cliArgs: CLIArgs): Promise<void> => {
	const bardsCurationToken = cli.contracts.BardsCurationToken
	...
	logger.info(`> Token: ${bardsCurationToken.address}`)
	...
	const allowance = (
		await bardsCurationToken.functions['allowance'](cli.wallet.address, disperseContract.address)
	)[0]
	if (allowance.gte(totalAmount)) {
		...
		await sendTransaction(cli.wallet, bardsCurationToken, 'approve', [
	...
	for (const batch of recipientsBatches) {
		queue.add(async () => {
			...
			for (const recipient of batch) {
				logger.info(
					`  > Transferring ${recipient.address} => ${formatGRT(recipient.amount)} (${recipient.amount
					} wei)`,
				)
			}
			try {
