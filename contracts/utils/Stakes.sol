...
/**
 * @title A collection of data structures and functions to manage the Delegator Stake state.
 *        Used for low-level state changes, require() conditions should be evaluated
 *        at the caller function scope.
 */
library Stakes {
    using SafeMath for uint256;
    using Stakes for DataTypes.Delegator;

    /**
     * @dev Deposit tokens to the Delegator stake.
     * @param stake Stake data
     * @param _tokens Amount of tokens to deposit
     */
    function deposit(DataTypes.Delegator storage stake, uint256 _tokens) internal {
        stake.tokensStaked = stake.tokensStaked.add(_tokens);
    }

    /**
     * @dev Release tokens from the Delegator stake.
     * @param stake Stake data
     * @param _tokens Amount of tokens to release
     */
    function release(DataTypes.Delegator storage stake, uint256 _tokens) internal {
     ...
     */
    function allocate(DataTypes.Delegator storage stake, uint256 _tokens) internal {
     ...
     */
    function unallocate(DataTypes.Delegator storage stake, uint256 _tokens) internal {
     ...
     */
    function lockTokens(
        DataTypes.Delegator storage stake,
        uint256 _tokens,
        uint256 _period
    ) internal {
     ...
     */
    function unlockTokens(DataTypes.Delegator storage stake, uint256 _tokens) internal {
     ...
     */
    function withdrawTokens(DataTypes.Delegator storage stake) internal returns (uint256) {
     ...
     * @return Token amount
     */
    function tokensUsed(DataTypes.Delegator memory stake) internal pure returns (uint256) {
     ...
     * it is not subject to change by a withdraw call from the Delegator.
     * @param stake Stake data
     * @return Token amount
     */
    function tokensSecureStake(DataTypes.Delegator memory stake) internal pure returns (uint256) {
        return stake.tokensStaked.sub(stake.tokensLocked);
    }

    /**
     * @dev Tokens free balance on the Delegator stake that can be used for any purpose.
     ...
     * @return Token amount
     */
    function tokensAvailable(DataTypes.Delegator memory stake) internal pure returns (uint256) {
        uint256 tokensCapacity = stake.tokensStaked;
        uint256 _tokensUsed = stake.tokensUsed();
        // If more tokens are used than the current capacity, the Delegator is overallocated.
        // This means the Delegator doesn't have available capacity to create new allocations.
        // We can reach this state when the Delegator has funds allocated and then any
        // of these conditions happen:
        // - The delegationCapacity ratio is reduced.
        // - The Delegator stake is slashed.
        // - A delegator removes enough stake.
        if (_tokensUsed > tokensCapacity) {
            // Delegator stake is over allocated: return 0 to avoid stake to be used until
     ...
     * @return Token amount
     */
    function tokensWithdrawable(DataTypes.Delegator memory stake) internal view returns (uint256) {
